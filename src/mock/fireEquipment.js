...
let database = fireEquipment.numbers
...
module.exports = {
  [`GET ${apiPrefix}/monitor/subsystem/fire/equipmentManage`] (req, res) {
    const { query } = req
    let { ...other } = query
    let newData = database
    if (other == null){
      res.status(200).json(fireEquipment)
    }else{
  ...
  [`POST ${apiPrefix}/monitor/subsystem/fire/equipmentManage`] (req, res) {
  ...
  [`GET ${apiPrefix}/monitor/subsystem/fire/equipmentManage/:id`] (req, res) {
    const { id } = req.params
  ...
  [`DELETE ${apiPrefix}/monitor/subsystem/fire/equipmentManage/:id`] (req, res) {
    const { id } = req.params
    const data = queryArray(database, id, 'id')
    if (data) {
  ...
  [`PATCH ${apiPrefix}/monitor/subsystem/fire/equipmentManage/:id`] (req, res) {
    const { id } = req.params
