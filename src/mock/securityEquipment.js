...
let securityEquipment = Mock.mock({
  'numbers|20-25': [
    {
      // name:'@cword(2)',
      'name|1': [
...
let database = securityEquipment.numbers
...
module.exports = {
  [`GET ${apiPrefix}/monitor/subsystem/security/equipmentManage`] (req, res) {
    const { query } = req
    let { ...other } = query
  ...
  [`POST ${apiPrefix}/monitor/subsystem/security/equipmentManage`] (req, res) {
  ...
  [`GET ${apiPrefix}/monitor/subsystem/security/equipmentManage/:id`] (req, res) {
    const { id } = req.params
  ...
  [`DELETE ${apiPrefix}/monitor/subsystem/security/equipmentManage/:id`] (req, res) {
    const { id } = req.params
    const data = queryArray(database, id, 'id')
    if (data) {
  ...
  [`PATCH ${apiPrefix}/monitor/subsystem/security/equipmentManage/:id`] (req, res) {
    const { id } = req.params
